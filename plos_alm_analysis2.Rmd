...
Create a cite_count vector by summing the 3 citation count variables together and produce summary statistics:
```{r}
...
The summary statistics show that the average number of citations per paper is 17.  There is a long right tail meaning that there are some outliers that have a very high number of citations (with a max of 3440).  This can be seen in the boxplot below.
...
```
There are a large number of papers with no citations, a slightly higher number with 1-10 citations, and even higher amount with 10-100 citations.  There are a small number of outliers that have a very high number of citations >100.  This is shown by the barplot below.
```{r fig.height=3, fig.width=6}
